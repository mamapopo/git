<template>
    <div id='news'>
        <mynav></mynav>
        <header>
        <img id='mamaimg' v-show="imgsrc" :src="imgsrc"/>
    </header>

    <div class="detail-content" v-html="mynew" v-cloak></div>
...
<script>
    ...
    import mynav from '@/components/common/news-nav' 
    ...
    export default{
       ...
       components:{
        ...
        .then((res)=>{
            ...
            this.mynew=res.data.body;
            this.css=res.data.css[0];
            console.log(this.css);
        //    document.getElementById('news').innerHTML(this.news);
        })
        .catch((err)=>{
            console.log
            ('start',err,'end');
...
<style >
/*scoped只对当前组件有效，相当于v-html里远端传入的也无效*/

#mamaimg{
    width: 100%;
    height:2rem;
    box-shadow: 0 10px 20px #292929;
